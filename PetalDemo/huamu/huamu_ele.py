...
class HuaMuEle(Task):
  ...
  def run(self):
    ...
    target_df["安装日期"] = target_df["安装日期"].replace(np.nan, 0).replace(np.inf, 0).astype("int").replace(0, np.nan)
    ...
    if not res_inner.empty:
      ...
      res_inner["下次检验日期"] = res_inner["下次检验日期"].replace(np.nan, 0)
      res_inner["下次检验日期"] = res_inner["下次检验日期"].replace(np.inf, 0).astype("int").replace(0, np.nan)
