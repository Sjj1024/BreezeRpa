...
def get_url(soup):
    print("提取url")
    main_container = soup.select("div.content")[0]
    category_container = main_container.select("div.grid-item")
    category_dict = {}
    for index, category in enumerate(category_container):
        title = category.select("div.card-header > a")[0].get_text().replace(" ", "").replace("\n", "")
        desc = category.select("span.pull-right")[0].get_text()
        category_wrapper = category.select("div.card-body div.website")
        a_link_list = []
        for link in category_wrapper:
            a_text = link.select("a.track")[0].get_text().replace(" ", "").replace("\n", "")
            a_link = link.select("a.track")[0].get("href")
            print(f"分类：{title}, 网站：{a_text}, 链接：{a_link}")
