import pandas as pd
from PetalDemo.collector_utils import get_template_client_base
...
class HuaMuMidFunds(object):
  ...
  def get_funds_total(self, data: pd.DataFrame, mid_df: pd.DataFrame):
    ...
    for index, row in agg.iterrows():
      mid_df = mid_df.append([{"所属党组织": index[0],
                               "所属党组织编号": index[1],
                               "党费收缴总额": row["本月实缴金额"][0]}])
    return mid_df
  ...
  def get_paid_person(self, data: pd.DataFrame, mid_df: pd.DataFrame):
    """
    获取已缴纳人次:  本月实缴金额>0 的统计人数
    :return:
    """
    # 筛选>0的人
    data = data[data["本月实缴金额"] > 0]
    # count统计
    agg_res = data.groupby(["所属党组织"]).agg({"党员编号": ["count"]})
    for index, row in agg_res.iterrows():
      mid_df.loc[index, "已缴纳人次"] = row["党员编号"]["count"]
    return mid_df
