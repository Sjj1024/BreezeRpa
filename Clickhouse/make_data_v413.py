...
def make_date(index, count, sub_target, random_data):
    target_data = []
    index_num = count + (index * sub_target)
    while True:
        index_num += 1
...
def start_run():
    ...
    client = clickhouse_connect.get_client(host=host, port=port, username=username, password=password)
    count = client.command(f"select max(toInt32(id)) from {data_name}.{table};")
...
if __name__ == '__main__':
    ...
    password = "ck@12345"
    # 要向哪个库的哪个表里面创建数据
    data_name = "p_6585760666659852288"
    table = "d_6585823149625970688"
